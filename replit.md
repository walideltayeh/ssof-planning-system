...
- **Multi-flavor clearance basket + cycles-to-clear (May 2026 rework):** every deck now carries a multi-flavor *basket* of slow MC instead of pairing the anchor with one slow SKU. The shared `clearanceBasket(slowList, targetMc, k, maxFlavors)` helper allocates MC across the top-N slow SKUs proportional to each one's overhang share, with a minimum of 1 MC per flavor and a per-SKU cap at the actual `closingStock` (so we never promise more than we have). Each builder tunes `targetMc` and `maxFlavors` to its template (Ride-Along ≈ anchor MC across 6 flavors, Variety = anchor MC across 6, Subscription = `weeklySlowTarget×4` across 4, Cafe = 2 MC across 2, Territory = 20 MC across 6). Two derived numbers ride into every bundle/script via `depletionInfo` + `depletionBundleRow`: **Total dead stock targeted** (sum of all eligible slow `closingStock` × $/MC) and **Cycles to clear it all** (`⌈totalSlowMc / basketMc⌉` — how many times this same offer needs to run, across the rep's accounts, to drain the country pile). The phone scripts now name the cycles-to-clear explicitly ("run ~3× across your peers and the warehouse drains") which is the single biggest "irrational to say no" lever. All builders return `Deck | null` and `buildAllDecks` filters nulls (covers the edge case where every basket candidate has zero usable stock).
- The 5 deck templates (every quantity is in **mastercases / MC** — the single warehouse unit):
  1. **🏍️ The Ride-Along** — simplest piggyback. `N` MC bestseller + a multi-flavor slow basket sized ≈ N MC across up to 6 flavors at the same per-MC price (`N` = clamp(⌈100/mixPct⌉, 5, 15)). Default play.
  2. **🎁 The Variety Builder** — `M` MC bestseller + a variety basket ≈ M MC across up to 6 flavors + a $50-per-flavor Instagram launch fund. Only renders when ≥2 slow SKUs exist.
  3. **🔁 The Subscription Lock** — 4-week recurring program: each week ships `⌈N/2⌉` MC bestseller; across the program a `⌈N/2⌉×4`-MC FREE slow basket is delivered across up to 4 flavors. Smooths cash flow, locks the shelf for a month, the slow basket is the loyalty gift.
  4. **☕ The Café Starter Pack** — 3 MC bestseller + a 2-MC sample basket across up to 2 flavors + a free Friday-night hookah-master demo (worth $150). Smallest-entry play for small accounts.
  5. **👑 The Territory Exclusive** — 20 MC bestseller + a 20-MC clearance basket across up to 6 flavors + 90-day exclusive territory rights for the *lead* basket flavor (largest piece) + quarterly business review. For master distributors.
